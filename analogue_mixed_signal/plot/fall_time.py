...
file_name = 'fall_time.csv'
component_tested = 'Fall Time'
...
start_point = steady_state2 + 0.1*steady_state_differential
start_point_index = voltage_smooth.index(min(voltage_smooth, key=lambda x: abs(x - start_point)))
end_point = steady_state2 + 0.9*steady_state_differential
...
plt.text(4.5E-06, 0, f'Δt = {round(time_differential,12)} [s]')
plt.xlabel('Time (s)')
